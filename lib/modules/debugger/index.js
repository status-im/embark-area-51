...
class TransactionDebugger {
  constructor(embark, _options) {
    ...
    embark.events.on('blockchain:tx', (tx) => {
      embark.events.request("contracts:contract", tx.name, (contract) => {
        ...
        if (tx.status === '0x0') {
          embark.logger.info("Transaction failed");
  ...
  listentoAPI() {
    ...
    this.embark.registerAPICall('post', '/embark-api/debugger/JumpBack', (req, res) => {
      this.cmd_line.debugger.step_manager.stepJumpNextBreakpoint()
    })
    this.embark.registerAPICall('post', '/embark-api/debugger/JumpForward', (req, res) => {
      this.cmd_line.debugger.step_manager.stepJumpPreviousBreakpoint()
    })
