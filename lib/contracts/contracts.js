...
class ContractsManager {
  constructor(options) {
    ...
    for (let className in self.contractsConfig.contracts) {
      plugin.registerAPICall(
        'get',
        '/embark/contract/' + className,
        (req, res) => {
          self.events.request('contracts:contract', className, res.send.bind(res));
        }
      );
    }
    plugin.registerAPICall(
      ...
      '/embark/contract/',
      (req, res) => {
        self.events.request('contracts:contract', req.query.filename, res.send.bind(res));
      }
    );
  }
